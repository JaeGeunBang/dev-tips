...
허나 위와같이 하면 timestamp를 dropDuplicates()에 넣기 때문에 중복 제거가 제대로 동작하지 않는다. 이를 위해 timestamp는 window()로 묶은 후 진행한다.

```scala
val sparkSession = LocalSparkSession()
...
.dropDuplicates("A", "B", "C", "window")

```

위와 같이 수행하면 timestamp가 window로 묶이며, 이 사이에서 중복을 정상적으로 제거할 수 있다.
